...
`;
  ...
  if (isAdvanced) {
    return `
Ты персональный тревел-гид Go Travel. Составь насыщенный маршрут на 1 день в городе "${city}" под настроение "${mood}".

Учитывай:
— атмосферу города,
— редкие и интересные локации,
— места для еды и отдыха,
— вечерние события (если есть).
${baseRules}
`;
  }
...
function formatCategory(code) {
 const map = {
    ...
    shopping: "🛍 Шопинг",
    food: "🍽 Еда",
    relax: "🧘 Отдых",            // ← добавь вот это
    adventure: "🚵‍♂️ Приключения" // ← и любые другие, которые GPT может вернуть
  };
